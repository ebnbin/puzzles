...
- **`placeSingles`** 反过来,读候选,写值:候选只剩一个的格子填上那一个。
- **`clearMarks`** 全部擦掉,这是让第一个键重新变回「填」的唯一办法。

**前两个必须指向相反方向**,这是设计而不是巧合:谁都喂不到自己,所以连按两次第二次一定
什么都不做;交替按才互相喂。在 `placeSingles` 里加一道「填之前先检查约束」看起来很自然,
恰恰不能做——那会让两个键互相读,也就是这个形状要避免的循环。

**线画在哪:只抄棋盘已经说过的话。** 行列宫、笼子的和、不等号、可见塔数、怪物总数——都是
「这一格能是什么」。「这个数字只能放在这一格」是另一回事,是**推断**,而推断就是上游的难度
设置本身(`DIFFLIST` 每一条就是一个 solver 函数)。所以这对键走不远:实测每个预设发五局,
Solo 2x2 Trivial 和 Undead 4x4/5x5 Easy 全解开,Solo 3x3 Trivial 五局中两局,Advanced
往上只走完几个百分点的空格,Killer 是零。它是记账的助手,不是解题器。
