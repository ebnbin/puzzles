...
用 **rebase merge**,历史保持线性。不要 squash,更不要 merge 提交——一个提交一件事,
压成一条就没法按提交回看了。
...
- **localStorage 的 key**(`src/engine/saves.ts` 列全了,外加 `puzzles.theme`、
  ...
  `light`,否则两边会对同一个从没选过的读者给出不同答案。`puzzles.arrows` 是第二个例子:
  它存过「开了方向键的游戏名数组」,后来方向键变成全局开关,同一个 key 改存 `'true'`/`'false'`,
  只认 `'true'`——老的数组读出来自然是关,正好是默认值,谁也不用迁移。
...
提交信息用中文,保持简洁:标题一句话说清这次改了什么;只有代码看不出来的理由才值得写进
正文,能一行就一行,没有就不写。
