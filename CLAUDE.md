...
- **提交的 author 是 owner(`Bin Zhang <ebnbin@gmail.com>`),committer 保持 Claude。**
...
- `vendor/sgtpuzzles/` 是上游的逐字副本,**一行都不许改**;一切行为改动都在外面的
  JS/TS 层做。
- **上游钉死,不写「上游改了怎么办」的兜底。** 申报里的上游事实(控件、偏好 kw、键码、
  调色板槽)以生成物 `src/games/facts.ts` 为准——从引擎录,不手抄;绑定用上游自己的 id
  (偏好按 kw、自定义参数按位置),显示文案只是翻译表的键。申报和引擎不一致就 throw,
  不退让、不静默降级。上游真要升级,走专门的迁移:重编、重录 facts、重核申报。
