...
| guess | ✅ | `Enter` 放置／提交 ✅ · `Space` 保留 ✅ · 需要一份光标副本 · 数字放 peg、`⌫`/`d` 删⭕拖拽都能做，见下 | `h`/`?` 提示 ✅ · `l` 标签开关⭕在偏好设置里 |
| pegs | ⬜ | `Enter` 选中棋子/跳 | —— |
...
得把每一条产生同一步棋的路都走一遍，而不是找到一条就下结论。guess 的数字键是同一个错误的第二
次，见「guess 的数字键」——**两次都是只读了 `interpret_move` 里键盘那半边。**
...
这一段以前写的是「数字键和 `⌫`/`d` 还差，是一件独立的活」。**记错了，三样都够得着。**

- **放一枚颜色**：从左边那条颜色柱**拖**到棋子位上（guess.c:857-859、888-892）。实测触摸
  下成立：390×844 和 320×568 两档都能放进去，颜色柱上每一格分别是 45 和 27 CSS px。
- **删一枚**：捏住已经放好的那枚**拖出这一行**再松手（`drag_opeg > -1` 走
  `set_peg(..., 0)`，guess.c:895-899）。两档都实测过。
- **`l`（给颜色标数字）**：它是一条真的偏好设置（`PREF_SHOW_LABELS`，guess.c:442），所以
  它早就在偏好设置弹窗里了——那一栏写着「Label colours with numbers」。

而且拖拽还多干一件键盘干不了的事：从**上面某一行旧猜测**里拖一枚颜色下来
（guess.c:872-878）。

按第一条判据，三样都不给按钮。按第二条也一样：**方向键把光标移到棋子位上之后，本来就有东西
可按**——上下键移的是 `ui->colour_cur`，棋盘会在选中的那枚颜色上画一个光标圈
（guess.c:1420-1426），`Enter` 放的就是它。光标不靠数字键才有意义。

**还有一条是数字键自己的毛病，和判据无关：它和棋盘上那个光标圈会各说各的。** 数字键写完格子
会把 `peg_cur` 往前推一格，却从不动 `colour_cur`（guess.c:940-947）。实测：光标停在第 0 位，
按 `3` 落下绿色，再按 `Enter`，第 1 位落下的是**红色**——颜色圈根本没跟着走。键盘上无所谓
（是你自己敲的 `3`），但一排彩色按钮摆在一块正画着另一个圈的棋盘旁边，屏幕上就有两个东西同时
在回答「现在选的是哪个颜色」，而且答案不一样。

这是「看见一个键映射到某个动作，不等于那个动作只有它能做」的第二个例子，第一个是 mines 的连开
（见「已经踩过的坑」）。**两次都是只读了键盘那半边的 `interpret_move` 就下了结论。**
