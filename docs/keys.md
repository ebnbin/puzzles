...
**四十个走完了。** 剩下的都不是「某个游戏还没做」，而是几笔跨游戏的旧账，各自记在下面：
loopy 的中键、九个游戏的修饰键组合、galaxies 的 `h`、map 的 `l`、bridges 的 `g`、
signpost 的 `x`、inertia 的 `Enter`。它们全是第 2 类（和光标无关），判据一下都够不着，
但每一个都要么需要一个新的机制（修饰键开关），要么需要键盘那一排多一个键。
...
| signpost | ✅ | `Enter` 连到下一格 ✅ · `Space` 连到上一格 ✅（开着连接时两个键都能取消，见 `BOTH`）· `x` 标记不可能⬜ | —— |
