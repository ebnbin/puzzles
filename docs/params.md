...
> 所以界面上调的不是 e,是归一化的**粒度 t ∈ [0, 1]**,e 由公式算出来交给上游(控件里存的仍是上游的 e):`M = max(宽, 高)`、`base = M − (M − 2)·t`、`e = M/base − 1`。t = 0 是 base 就是棋盘本身(矩形最碎),t = 1 是 base 缩成 2×2(矩形最少),中间线性走 base 的长边。同一个 t 在不同棋盘上 e 不同——这正是它跨棋盘对齐粒度的办法。实测 t = 0.25 / 0.50 / 0.75 时矩形数是 t = 0 的 67–73% / 43–53% / 21–25%,7×7、19×19、50×50、30×10 四种棋盘一致。
