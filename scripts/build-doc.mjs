...
;(function () {
  ...
  document.addEventListener('click', function (event) {
    var link = event.target.closest && event.target.closest('.doc-lang')
    if (!link) return
    try {
...
function topBar(lang, file) {
  const c = CHROME[lang]
  /*
   * One link, to this page in the other language, labelled with that
   * language's own name — the app's control (see src/lang-switch.css, which
   * both stylesheets include) and the same shape as the button beside it.
   *
   * It was two links in a segmented switch, showing both languages and marking
   * the current one. A link is better than a button was in the app, and it is
   * better here too: this page exists twice, so the other language is an
   * address and not a state to be set. Middle-click it, open it in a tab, copy
   * it — all of which a `<button>` could never have offered.
   *
   * `doc-lang` is also what the head script listens on, to keep the app's idea
   * of the language in step when it is changed from here.
   */
  const other = lang === 'en' ? 'zh' : 'en'
  const o = CHROME[other]
  return `<header class="doc-top">
<a class="doc-home" href="/">${BACK_ICON}${c.home}</a>
<span class="doc-top-title">${c.section}</span>
<a class="doc-lang lang-switch" href="/doc/${o.dir}${file}" data-lang="${other}" hreflang="${o.htmlLang}" lang="${o.htmlLang}">${LABEL[other]}</a>
 ...
 * rules. lang-switch.css is here rather than copied into doc.css because a
...
const css =
  ...
  '   src/lang-switch.css and src/doc.css. Edit those, not this. */\n' +
  ['src/tokens.css', 'src/lang-switch.css', 'src/doc.css']
