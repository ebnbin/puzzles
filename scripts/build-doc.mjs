...
// 这个字符串随每个手册页原样发布,里面的 // 注释是产物内容,不归本仓库的注释规矩管。
// 主题/语言解析和 index.html 内联脚本是同一段,联动改。
...
const css =
  '/* Generated by scripts/build-doc.mjs from src/tokens.css, src/segmented.css,\n' +
  '   src/doc-page.css and src/doc.css. Edit those, not this. */\n' +
  ['src/tokens.css', 'src/segmented.css', 'src/doc-page.css', 'src/doc.css']
    .map((f) => readFileSync(join(ROOT, f), 'utf8'))
    .join('\n')

writeFileSync(join(OUT, 'doc.css'), css)
// doc.css 是同一地址原地重写,必须带内容摘要 query:service worker 按整条 URL 存,vercel 对它写
// immutable 也只因这个 query 才合法。
