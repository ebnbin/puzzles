...
// 这三个的 Solve 只展示答案不改盘(flood 存的是 solution path),直接用发牌图;cube/samegame/pegs
// 的 can_solve=false 由「solve 前后像素相同即回退」兜住,不用列在这里。
  ...
  for (const theme of THEMES) {
    ...
    try {
      ...
      if (SOLVE_ONLY_SHOWS.has(game.name)) {
        ...
        // 取开局必须 loadGame(上游 .sav)后 restart():restart 保留生成时的 aux,netslide/untangle
        // 没有 aux 会拒绝 solve。
