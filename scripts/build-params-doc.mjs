...
const MECHANISM = [
  ...
  ['1.3 控件', [
    ...
    '**choices 与 boolean**:choices 画成分段按钮(外观同首页的语言切换,会换行),选项全部可见、一点即换,写回的仍是选项下标;申报了 `ordinal` 的 choices(Bridges 的桥数、岛占比、扩展因子,上游用下拉装的数值阶梯)画成滑块,一档一个选项。boolean 是勾选框。两类的取值就是上游给的,不进范围模型。「门」是下游自己加的开关,挂在一个 int 控件上(Sixteen 的打乱步数):关 = 写 0、滑块置灰,开 = 写 1,状态从数字读出,没有上游控件。',
