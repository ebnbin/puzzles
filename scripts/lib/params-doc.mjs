  ...
  },
  {
    ...
    params: [
      ...
      { label: 'Rotating block size', kind: 'int', sem: '一次旋转的方块边长;点一下把一个 n×n 块整体转 90°',
        u: ['≥ 2(214)', '≤ 宽且 ≤ 高(216-218)'], f: '2..min(宽, 高, 30)', d: '看宽高;宽高缩小时被夹', c: 'local' },
      { label: 'Number of shuffling moves', kind: 'int',
        sem: '0 = 默认长打乱;N > 0 = 只做 N 次旋转,状态栏挂 target(语义同 Sixteen)',
        u: ['≥ 0(222)', '无上限'], f: '0..100', d: '—', c: 'local' },
    ],
    notes: [
      '宽高的上限和 Fifteen 同据(字号 = 格边/3,twiddle.c:1005)。块边长的 30 是耗时定的:打乱步数为 0(默认值、全部八条预设)时,'
        + '上游走 w·h·n²·2 步、每步 do_rotate 是 O(n²)(twiddle.c:340),整体 O(w·h·n⁴)。'
        + '浏览器里 50×50 逐档实测:n=10 落定 0.39 秒、n=20 三秒、n=30 9.5 秒、n=40 21.6 秒、n=50 140 秒。30 以下的盘子不受这条影响,min(宽, 高) 先生效。',
      ...
      '唯一非单调的是 n = 宽 = 高:只有一个旋转位置,整场打乱退化成整盘 ±90° 的随机游走,净转动 ≡ 0 (mod 4) 时 grid_complete 成立、'
        + '外层把整场打乱作废重来,而防撤销的过滤器在这种情况下被显式关掉(twiddle.c:365)。同样 n=20、60 个种子:20x20n20 平均 0.324 秒、最坏 1.454 秒,'
        + '而盘子更大的 21x21n20 是 0.247 / 0.253。尾巴是几何分布、没有上界;在 n ≤ 30 的范围内基数够小,不构成问题。',
...
export const DEVIATIONS = [
  { game: 'twiddle', what: '宽高封到 50,旋转块边长封到 min(宽, 高, 30)', why: '宽高同 Fifteen(字号 = 格边/3)。块边长:默认打乱是 O(w·h·n⁴),浏览器里 50×50 的 n=30 落定 9.5 秒、n=40 21.6 秒、n=50 140 秒。上游只要求 2 ≤ n ≤ min(宽, 高)。' },
