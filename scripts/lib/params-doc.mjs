  ...
  },
  {
    ...
    params: [
      ...
      { label: 'Expansion factor', kind: 'float',
        sem: '先在一张 base 棋盘上把矩形铺满,再插行插列撑回原尺寸(矩形个数和相邻关系不变,只是每个变大)。base 边长 = ⌊边/(1+e)⌋,不足 2 时钉 2(1165-1168);0 = base 就是棋盘本身。界面上换成了粒度 t,e 由 t 算出',
        u: ['≥ 0(229)', '无上限', 'atof 解析,%g 回显'],
        f: '粒度 t = 0.00..1.00,步长 0.01(101 档);写进控件的是换算出的 e = 长边/base − 1,四位小数,0..长边/2 − 1', d: '看宽高(长边)', c: 'local' },
    ],
    notes: [
      ...
      'base 缩到最小(2×2)之后 e 再大也逐字节相同,那一点是 e > max(宽, 高)/3 − 1(边长掉到 2 需要 边/(1+e) < 3),'
        ...
        + '滑块从 0 拉到头在小棋盘上大半程是同一个局面。',
      '所以界面上调的不是 e,是归一化的**粒度 t ∈ [0, 1]**,e 由公式算出来交给上游(控件里存的仍是上游的 e):'
        + '`M = max(宽, 高)`、`base = M − (M − 2)·t`、`e = M/base − 1`。t = 0 是 base 就是棋盘本身(矩形最碎),'
        + 't = 1 是 base 缩成 2×2(矩形最少),中间线性走 base 的长边。同一个 t 在不同棋盘上 e 不同——这正是它跨棋盘'
        + '对齐粒度的办法。实测 t = 0.25 / 0.50 / 0.75 时矩形数是 t = 0 的 67–73% / 43–53% / 21–25%,'
        + '7×7、19×19、50×50、30×10 四种棋盘一致。**t 是这一档的真身**:表里第 k 档就是 t = k/100 算出来的那个 e,'
        + '读数的 t 按下标认回来,e 和 base 都是算出来的。',
      '面板上读数是 t,轨道下面单独一行摊开 `e 0.9607 · base 51×5`:base 是照 rect.c:1165-1168 那两句'
        + '(单精度、截尾、不足 2 钉 2)算的,所见即上游真正会用的那一对。小棋盘上能一眼看出 101 档里有大片是同一对 base'
        + '——7×7 只有 6 对(7,6,5,4,3,2),真正不同的局面只有六种。',
      't 的步长 0.01 是定的,e 保留四位小数并**只往小里截**(截大了 ⌊M/(1+e)⌋ 会掉到 base 下面一格)。'
        + '两位小数不够:小棋盘上相邻两档会算出同一个 e,表就不严格递增了。100×100 上 101 档给出 99 个互不相同的 base;'
        + '单看一步 0.01 对矩形数的影响(100×100 平均 377.1 → 371.6,1.5%)小于种子之间的抖动(314..467),'
        + '要看出差别得拉几档——这是 owner 定的粒度。',
      '改宽高会按新长边重算这张表,存着的 e 被吸到最近一档,**t 会漂**:100×100 的 t = 0.50(e = 0.9608)改成 10 宽之后落在 t = 0.61。'
        + '范围模型只认得数值、认不得「上一次的 t」,这是换算方案的代价。',
...
export const DEVIATIONS = [
  ...
  { game: 'rect', what: '扩展因子换成粒度 t = 0.00..1.00(步长 0.01),e 由 t 算出', why: 'e 的有效区间跟着棋盘长边走(到 max(宽,高)/2 − 1 为止,再大是同一局),同一个 e 在不同棋盘上粒度差很远。t 归一化之后跨棋盘对齐,顶到 base = 2×2 那一点;上游只要求非负。' },
  { game: '(浮点)', what: '障碍概率步长 0.01、扩展因子按粒度 t 的 101 档', why: '滑块只能取有限个值;上游接受任意小数。Game ID 载进来的表外小数,第一次落定时吸到最近一档。' },
