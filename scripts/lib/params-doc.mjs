  ...
  },
  {
    ...
    params: [
      ...
      { label: 'Expansion factor', kind: 'float',
        sem: '先在一张 base 棋盘上把矩形铺满,再插行插列撑回原尺寸(矩形个数和相邻关系不变,只是每个变大)。base 边长 = ⌊边/(1+e)⌋,不足 2 时钉 2(1165-1168);0 = base 就是棋盘本身',
        u: ['≥ 0(229)', '无上限', 'atof 解析,%g 回显'],
        f: '0.00..33.00,步长 0.05', d: '—', c: 'local' },
    ],
    notes: [
      '宽高从 2 起是**避崩**,不是调优:上游只查 w·h ≥ 2,放行 1×n;但钳位那句写的是 `if (bw < 2 && w >= 2)`,'
        + '宽是 1 时不触发,base 边长留在 0,扩展因子一动就死——真机上 1×7 加一档(0.05)即 wasm abort'
...
export const DEVIATIONS = [
  ...
  { game: 'rect', what: '扩展因子封顶 33.00、步长 0.05', why: 'base 缩到最小 2×2 之后 e 再大也逐字节相同,那一点是 max(宽, 高)/3 − 1;100 宽时是 32.3。上游只要求非负。' },
