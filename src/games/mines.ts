// Mines:扫雷。上游 mines.c。Enter 开格/连开、Space 插旗/取旗,脸跟着标签换。Clear 亮着不等于
// 安全(旗插错了照样亮),不修。
import type { Game } from './game'
import { still } from './game'
import type { Custom } from './util/custom'
import { AREA_MAX, height, rule, width } from './util/custom'
import { samePages, verbatim } from './util/declare'
import { act, cross } from './util/pad'

// validate_params mines.c:274-323:宽高 ≥ 1(292),确保有解时都 > 2(290,full);雷数 1 到
// 面积 − 9(307-310)。上游文本框接受的 "n%" 写法 slider 不会产生。
const custom: Custom<'mines'> = {
  fields: [
    width(1),
    height(1),
    { kind: 'int', key: 'n', word: 'mines', min: 1, max: AREA_MAX - 9, role: 'count' },
    { kind: 'flag', key: 'unique', word: 'soluble' },
  ],
  rules: [
    rule('mines.c:290', ['w', 'unique'], (v) => !!v.unique && v.w <= 2),
    rule('mines.c:290', ['h', 'unique'], (v) => !!v.unique && v.h <= 2),
    rule('mines.c:309', ['n', 'w', 'h'], (v) => v.n > v.w * v.h - 9),
  ],
}

const mines: Game<'mines'> = {
  id: 'mines',
  upstream: { labels: 'live', cursor: { kind: 'reported' } },
  touch: { hold: 'right' },
  dark: { relief: [[16, 17]] },
  pages: samePages('mines'),
  types: { menu: verbatim, custom },
  prefs: { panel: verbatim, volatile: false },
  keypad: () => [],
  arrows: {
    keys: [
      ...cross(),
      act({
        id: 'uncover',
        slot: 4,
        key: 'Enter',
        idle: { glyph: 'uncover', word: 'uncover' },
        faces: {
          Uncover: { glyph: 'uncover', word: 'uncover' },
          Clear: { glyph: 'chord', word: 'chord' },
        },
      }),
      act({
        id: 'flag',
        slot: 6,
        key: ' ',
        idle: { glyph: 'flag', word: 'flag' },
        faces: {
          Mark: { glyph: 'flag', word: 'flag' },
          Unmark: { glyph: 'flag', word: 'unflag' },
        },
      }),
    ],
  },
  observe: still,
}

export default mines
