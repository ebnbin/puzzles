// 文案是数据,住在 en.json / zh.json;这里只有语言状态和 fill()。两份 JSON 的键集与占位符集由
// ./verify.ts 在构建期对账。有意不译的约定(JSON 放不下注释):brand 和 tagline 保持英文;键面、
// 长按提示这类中文优先取 doc-zh 手册对应章节的既有译法,不自创同义词。
...
// 默认页显式 index.html,不改成目录式 URL:halibut 把 Contents 链接写成 index.html,SW 按整条 URL 缓存。
