// 完成判定:一局走到头(解出或输掉)抬起收尾浮层,玩家自己解出时再记一笔。触发点是 onUndoRedo:
// emcc.c 的 post_move() 在每次输入后都发它,是 JS 侧唯一「状态可能变了」的信号。
// 浮层认「结束」(求解器解出的也抬),记录认「玩家自己解出」。一律只认沿,不认状态:一局的第一次
// 观察永远不算;回到进行中(undo 回去)就重新武装,浮层也收起来。
