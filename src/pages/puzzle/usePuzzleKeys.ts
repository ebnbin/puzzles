...
// 单独按下的修饰键不是走子:上游认不出它(key() 返回 false)。
const BARE = new Set([
  'Shift', 'Control', 'Alt', 'Meta', 'CapsLock', 'NumLock', 'ScrollLock',
])

// 上游的四个裸字母快捷键里补三个(midend.c:1005-1036);q 不补,emcc 拿到 PKR_QUIT 什么都不做。
// 按住 Shift 的那一次不算:emcc 给 Shift 的按键加上 MOD_SHFT(emcc.c:445),'N'|MOD_SHFT 匹配不上
// 'N';CapsLock 打出的 N 没有那一位,是快捷键。判据要和上游一模一样。
...
// 焦点当下归谁。'control' 只让出 Space / Enter,别的键让给谜题。
...
export function usePuzzleKeys({
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      // Ctrl + 单字符全是浏览器的快捷键,而上游把任何单字符连 MOD_CTRL 一起交给 midend
      // (emcc.c:402);Ctrl+方向照旧交给谜题(net 靠它移信号源)。
      ...
      // 先问引擎要不要,答 PKR_UNUSED 才轮到快捷键,同上游的判据。
